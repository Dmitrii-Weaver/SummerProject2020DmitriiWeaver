...
//taking functions from examples from the textbook
...
//applying them
    function dominantdir (given_text){
        counted_chars = countBy(given_text, char => {
            script = characterScript(char.codePointAt(0));
            return script ? script.direction : "none";
        }).filter(({name}) => name != "none");

        if (counted_chars.length == 0){
            return "ltr"
        } 
        else{
            return counted_chars.reduce((a, b) => a.count > b.count ? a : b).name;
        }
    }


console.log(dominantdir("sup user!"));

console.log(dominantdir(" لا شيء"));